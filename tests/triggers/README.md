...
| 2026-09-24 | Windows 11 Pro 26200 | any, with `DisallowChildProcessCreation`, started in a window of its own | Security-Mitigations/KernelMode | 4 | `child-process-block` (look) | The refused child was `conhost.exe` (`-ForceV1`), not the test's `cmd.exe`: a console program in its own window must start a console host, the mitigation refuses it, and the program dies at start with 0xC0000142. So event 4 is confirmed on this channel, and it names the refused child in `ChildImagePathName` / `ChildCommandLine`, but the `cmd.exe` path is not yet seen. `phase2-admin.ps1` now runs triggers in its own console and only accepts a `cmd.exe` child. |
| 2026-09-24 | Windows 11 Pro 26200 | `child` with `DisallowChildProcessCreation`, run in the script's console | Security-Mitigations/KernelMode | 4 | `child-process-block` (look) | `CreateProcessW` refused, exit 2. The event names the refused child: `ChildImagePathName` `C:\WINDOWS\SYSTEM32\cmd.exe`, `ChildCommandLine` `cmd.exe /c exit`. XML checked in, identifiers replaced. |
| 2026-09-24 | Windows 11 Pro 26200 | `lowil <Low-labelled copy>` with `BlockLowLabelImageLoads` | Security-Mitigations/KernelMode | 6 | `low-integrity-image-block` (look) | `LoadLibraryW` refused, exit 2. Unlike 2 and 8, the event names the refused image in `ImageName`, drive-less like CIG's (`\Witness\witness\tests\triggers\lowil-copy.exe`). XML checked in, identifiers replaced. |
| 2026-09-24 | Windows 11 Pro 26200 | `rwx`, `remote` in the same elevated run | Security-Mitigations/KernelMode | 2, 8 | `acg-block-kernel`, `remote-image-block` (urgent) | Re-confirmed; same shape as 2026-09-23. |
